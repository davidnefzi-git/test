...
\textit{Notes:} Each cell reports the fraction of total dollar supply to the
...
tightness and the premium, consistent with both channels in Proposition~1: demand
surged (foreign banks scrambled for dollars as offshore funding markets seized)
while dealer capacity contracted. Second, quarter-end spikes recur with clockwork
regularity, generating short-lived basis widening that dissipates within days.
The pattern at quarter-end is consistent with Proposition~2: capacity contracts
...
\textit{Notes:} The top panel plots the market-tightness proxy $\theta_t =
...
tightness and the premium is consistent with Propositions~1 and~2 and motivates
...
Table~\ref{tab:balance-tests} shows that MMF-exposed banks hold a dominant
position in most currency pair-tenor cells, consistent with the identification
assumption underlying Proposition~1. The balance tests confirm that
...
dollar-taking in the swap market. The balance tests in Table~\ref{tab:balance-tests}
...
A diagnostic test separates taker demand from maker demand. Proposition~1
...
$\hat\beta$ basis points (see Table~\ref{tab:iv-second-stage}, row ``Fitted
...
The structural interpretation follows from \cref{rem:price-impact}:
...
is consistent with Proposition~1 ($\partial\mu^*/\partial U > 0$) and confirms
...
(Proposition~2), quarter-end amplification (Proposition~4), and term-structure
concentration.

\paragraph{Dealer capacity amplification.} Proposition~2 predicts that the price
...
Proposition~2, though the interaction identifies effect heterogeneity across
...
of Proposition~4: dealer capacity contracts deterministically for regulatory
...
long-tenor markets. Table~\ref{tab:iv-interactions} Panel C presents estimates
...
in Table~\ref{tab:pretrend-diagnostic} regresses $\Delta\text{PriceUSD}_{m,t}$
...
rather than anticipated changes. Table~\ref{tab:iv-bootstrap} reports wild cluster
...
< \hat\beta_{HF}$ is consistent with the maintained hypothesis in
\cref{rem:beta-ordering}, though as noted there it is a maintained empirical
...
Table~\ref{tab:demand-system} reports the aggregate slope. The estimate is
negative and statistically significant. A negative and significant estimate of
$\mathcal{B}$ is consistent with the maintained hypothesis in Remark~1 and
