...
results follow: demand pressure raises the equilibrium premium (\cref{prop:demand}),
capacity tightening amplifies the effect (\cref{prop:capacity}), and the premium
responds convexly to market tightness under plausible conditions on the quality
function (\cref{prop:nonlinear}). The first two propositions map directly to the IV and
...
\paragraph{Testable prediction.} \Cref{prop:demand} is the primary empirical target.
...
\paragraph{Testable prediction.} \Cref{prop:capacity} maps to the interaction
...
consistent with \cref{prop:capacity} but does not establish the causal capacity
channel.
...
\emph{\Cref{prop:nonlinear} characterizes the theoretical shape of the price impact
...
present \cref{prop:nonlinear} as a theoretical result that motivates our focus on
...
The mechanism is the same as \cref{prop:capacity} but the supply-side variation is
