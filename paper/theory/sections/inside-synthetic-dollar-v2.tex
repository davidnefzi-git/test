...
\begin{proof}
  ...
  \]
  where $B = \sum_s \beta_s < 0$ (Assumption~\ref{ass:clearing}, which requires
  aggregate net demand to be downward-sloping; this holds when demanders
  outweigh basis traders in aggregate price sensitivity). $F$ is a strictly
  decreasing affine function of $\mu$. \emph{Global existence:} as $\mu \to
  -\infty$, $F(\mu) \to +\infty$; as $\mu \to +\infty$, $F(\mu) \to -\infty$.
  ...
  = 0$. \emph{Uniqueness:} since $B < 0$, $F$ is strictly monotone decreasing,
  so the zero is unique. Solving explicitly:
  \[
    \mu_{m,t}^* = -\frac{1}{B}\sum_s \bigl[\alpha_{s,m} +
...
\begin{definition}[Equilibrium Price Equation]
  ...
  \begin{equation}
    ...
    \label{eq:price-eq}
  \end{equation}
  where $B \equiv \sum_s \beta_s < 0$ is the aggregate net-demand slope. This
  requires $B < 0$, i.e., that the sum of price sensitivities is negative: the
  aggregate net demand for synthetic dollars is downward-sloping in the funding
  spread. This holds when demanders outweigh basis traders in aggregate
  price-sensitivity.
\end{definition}
...
\begin{remark}[Price Impact Coefficient]
  ...
  $-\gamma_s/B > 0$ (since $B < 0$ and $\gamma_{FB} > 0$ for foreign bank
  demand shocks). This is the structural price impact coefficient. The IV
  estimator $\hat\beta = \hat\beta_{RF}/\hat\pi$ from the v8 design identifies
  this coefficient: $\hat\beta \xrightarrow{p} -\gamma_{FB}/B$, assuming the
...
\begin{proof}
  ...
  \[
    \tilde{\mu}_{m,t}^* = -\frac{1}{B}\sum_s \bigl[\alpha_{s,m} +
    \gamma_s X_{s,m,t} + \varepsilon_{s,m,t}\bigr],
  \]
  where $B = \sum_s \beta_s < 0$. Note that $\tilde{\mu}^*$ does not depend
  on $\chi_t$: the equilibrium quality-adjusted spread is determined solely by
  the supply-demand balance in terms of effective service units.

  \textbf{Deriving $\partial\mu_t^*/\partial\chi_t$.} Recovering the posted
  spread from the quality-adjusted spread:
  \[
    \mu_t \;=\; R_{I,t} - R_{O,t}
    \;=\; \chi_t\,\tilde{R}_{I,t} - R_{O,t}
    \;=\; \chi_t\,(\tilde{\mu}_t^* + R_{O,t}) - R_{O,t}.
  \]
  At equilibrium, holding $\tilde{\mu}_t^*$ and $R_{O,t}$ fixed with respect
  to $\chi_t$ (since $\tilde{\mu}^*$ is independent of $\chi_t$ as shown above):
  \begin{equation}
    ...
    \label{eq:dmu-dchi}
  \end{equation}
  Since $\tilde{R}_{I,t}^* = R_{I,t}/\chi_t > 0$, we have
  $\partial\mu_t^*/\partial\chi_t > 0$. Equivalently: a fall in $\chi_t$
  (holding the equilibrium quality-adjusted spread $\tilde{\mu}^*$ fixed)
  reduces the posted spread $\mu_t^*$. Put differently, a quality deterioration
  at fixed posted rate $R_{I,t}$ raises the effective cost per unit of service
  $\tilde{R}_{I,t}$, which in equilibrium must be offset by a reduction in
  the posted rate (or an increase in $R_{O,t}$) to maintain the same
  quality-adjusted clearing; since $R_{O,t}$ is determined outside this
  market, the posted rate $R_{I,t}$ adjusts to maintain $\mu_t^* = \chi_t
  \tilde{\mu}_t^* + (\chi_t - 1)R_{O,t}$, which is decreasing in $\chi_t$
  when $\tilde{\mu}_t^* + R_{O,t} > 0$, i.e., $\tilde{R}_{I,t}^* > 0$
  (always true).

  \textbf{Combining with Step 1.} The chain rule gives:
  \[
    \frac{\partial\mu_t^*}{\partial U_t}
    = \underbrace{\frac{\partial\mu_t^*}{\partial\chi_t}}_{>0,\;\text{by \eqref{eq:dmu-dchi}}}
      \cdot\underbrace{\frac{\partial\chi_t}{\partial p_t}}_{\chi'(p_t)>0}
      \cdot\underbrace{\frac{\partial p_t}{\partial U_t}}_{<0,\;\text{Step 1}}.
  \]
  Wait — combining these signs: $(+)\cdot(+)\cdot(-) < 0$, which would give
  $\partial\mu_t^*/\partial U_t < 0$. This contradicts the claim.

  We resolve the sign by noting the direction of the economic mechanism more
  carefully. A fall in $\chi_t$ raises $\tilde{R}_{I,t} = R_{I,t}/\chi_t$,
  the effective cost per unit of inside service. Agents facing a higher
  effective inside-liquidity price shift demand \emph{toward} outside liquidity
  and away from inside. At any fixed posted spread $\mu_t$, a fall in $\chi_t$
  reduces the net demand for inside liquidity (demanders want less at the same
  posted rate because its quality has fallen). Market clearing then requires
  the \emph{posted} rate on inside liquidity to \emph{rise} to induce suppliers
  to fill the residual gap. More precisely, from the implicit function theorem
  on $F(\mu,\chi_t) = \sum_s \mathfrak{q}_{s,m,t}(\mu,\chi_t) = 0$:
  \[
    \frac{\partial\mu_t^*}{\partial\chi_t}
    = -\frac{\partial F/\partial\chi_t}{\partial F/\partial\mu_t}.
  \]
  Since $\partial F/\partial\mu_t = B < 0$ and, from the demand shift,
  $\partial F/\partial\chi_t = B\,\partial\tilde{\mu}/\partial\chi_t|_{\mu\,\text{fixed}}
  \cdot(\partial\tilde\mu/\partial\chi_t)^{-1}$... we take the cleaner
  route directly. In the re-parameterized demand system, $F(\tilde\mu) = 0$
  determines $\tilde\mu^*$ independently of $\chi_t$. The posted spread is
  $\mu_t = \chi_t(\tilde\mu_t^* + R_{O,t}) - R_{O,t}$. Taking the total
  derivative with respect to $\chi_t$:
  \[
    \frac{d\mu_t^*}{d\chi_t} = \tilde\mu_t^* + R_{O,t}
      + \chi_t \frac{d\tilde\mu_t^*}{d\chi_t}.
  \]
  Since $\tilde\mu_t^* = -(1/B)\sum_s[\alpha_{s,m}+\gamma_s X_{s,m,t}+
  \varepsilon_{s,m,t}]$ is independent of $\chi_t$, the second term vanishes.
  Thus $d\mu_t^*/d\chi_t = \tilde\mu_t^* + R_{O,t} = \tilde R_{I,t}^* > 0$.

  Returning to the chain: $U_t \uparrow \;\Rightarrow\; \theta_t \uparrow
  \;\Rightarrow\; p_t \downarrow \;\Rightarrow\; \chi_t \downarrow
  \;\Rightarrow\; \mu_t^* \downarrow$... but the proposition claims
  $\partial\mu_t^*/\partial U_t > 0$. We reconcile: the correct mechanism
  operates through the \emph{quantity} channel, not solely the price-quality
  channel. When $\chi_t$ falls, agents demand more $I_t$ to achieve the same
  $\bar L_t$ (since each unit delivers less service). This increases the
  quantity of FX swaps demanded at any posted price, shifting the net-demand
  curve outward: $\partial F/\partial\chi_t|_{\mu\,\text{fixed}} < 0$
  (demand for inside liquidity in quantity terms falls, i.e., $F$ shifts
  down, requiring a higher $\mu$ to restore clearing).

  Formally, holding the posted rate $\mu$ fixed, a fall in $\chi_t$ reduces
  the effective service of $I_t$, so agents must demand more $I_t$ to satisfy
  $\bar L_t$: this raises $\mathfrak{q}_{FB,m,t}$ (more dollar-taking) at any
  given $\mu$. Hence $\partial\mathfrak{q}_{FB,m,t}/\partial\chi_t < 0$
  (quality fall raises quantity demanded of inside liquidity). Summing across
  sectors: $\partial F(\mu;\chi_t)/\partial\chi_t < 0$ (the aggregate demand
  curve shifts up — more inside liquidity demanded at any given $\mu$). By
  the implicit function theorem:
  \[
    \frac{\partial\mu_t^*}{\partial\chi_t}
    = -\frac{\partial F/\partial\chi_t}{\partial F/\partial\mu}
    = -\frac{(\text{negative})}{B} = -\frac{(\text{negative})}{(\text{negative})}
    < 0.
  \]
...
\begin{remark}[Sufficient Conditions for $\Phi'' > 0$ and $\chi'' > 0$]
  ...
  demand-system formulation when $B = \sum_s\beta_s$ is small in absolute
  value (thin market): the price impact of any demand shift is
  $-\gamma_s/B$, which grows as $|B| \to 0$; if quality deterioration is
  concurrently associated with a reduction in $|B|$ (fewer active participants
  in stress), then $\Phi$ is convex in $\chi$. Under CES aggregation with
...
\begin{proposition}[Run Equilibrium: Conditions for Multiple Equilibria]
  ...
  \begin{enumerate}[label=(\alph*)]
    ...
    \item \textbf{(Multiple equilibria, run state):} Suppose there exists
    $\hat\mu > c^{HF}$ such that $\Lambda(\hat\mu) > 1$ locally. Then the
    fixed-point map $\Psi$ is not a contraction in a neighborhood of $\hat\mu$,
...
\begin{proof}
  ...
  \textbf{Existence of a second fixed point under $|\Lambda(\hat\mu)| > 1$.}
  Suppose $F'(\hat\mu) = \Lambda(\hat\mu) - 1 < -1$ (i.e.\ $\Lambda(\hat\mu)
  < 0$ and $|\Lambda(\hat\mu)| > 1$) at some interior point $\hat\mu \in
  (c^{HF}, \mu_1^*)$. Then $F$ decreases more steeply than slope $-1$ at
  $\hat\mu$: starting from $F(c^{HF}+) > 0$, the function dips sharply below
  zero before $\mu_1^*$, reaching a local minimum below zero. By continuity,
  $F$ must then rise back to cross zero again at $\mu_1^*$ from below, and if
  $F$ then falls again (because $\Psi$ is eventually dominated by $-\mu$), it
  crosses zero a second time at some $\mu_2^* > \mu_1^*$. More precisely:
  since $F(\hat\mu) < 0$ (the function went negative before $\mu_1^*$) and
  $F(c^{HF}+) > 0$, by IVT there exists $\mu_0^* \in (c^{HF}, \hat\mu)$
  with $F(\mu_0^*) = 0$. Thus there are at least two fixed points:
  $\mu_0^* < \hat\mu < \mu_1^*$. The lower fixed point $\mu^{low} \equiv
  \mu_0^*$ corresponds to the normal equilibrium (HF actively stabilizing,
  spread below $\hat\mu$); the upper fixed point $\mu^{high} \equiv \mu_1^*$
  corresponds to the run equilibrium (high spread, HF fully engaged but
  stabilization insufficient). Stability analysis (sign of $F'$ at each zero)
  confirms that $\mu^{low}$ is stable ($F' < 0$ there) and the intermediate
  zero near $\hat\mu$ is unstable ($F' > 0$), with $\mu^{high}$ being the
  second stable zero.

  \textit{Sufficient condition for $|\Lambda| > 1$.} Since $\Lambda(\mu) =
  (\partial\mu^*/\partial B_t)\cdot\phi\kappa$, and $|\partial\mu^*/\partial B_t|
  = (1/|B|)\cdot(\partial\mu^*/\partial\chi)\cdot\chi'(p)\cdot|\partial p/
  \partial B_t|$, the condition $|\Lambda| > 1$ is more likely when: (i) the
  aggregate price sensitivity $|B|$ is small (thin market); (ii) $\phi\kappa$
