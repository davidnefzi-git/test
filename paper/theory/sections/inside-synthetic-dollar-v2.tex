...
\begin{proof}
  ...
  \textit{Step 3 (Spread determination).} The equilibrium spread $\mu_t^*$ is
  determined by the market-clearing condition together with the sectoral demand
  system. Substituting the sectoral net-demand functions
  $\mathfrak{q}_{s,m,t}(\mu_{m,t})$ into \eqref{eq:clearing} and differentiating
  gives (see \cref{sec:demand-system}):
  \[
    \mu_{m,t}^* = -\frac{1}{B}\sum_s \bigl[\alpha_{s,m} +
    \gamma_s X_{s,m,t} + \varepsilon_{s,m,t}\bigr],
  \]
  where $B = \sum_s \beta_s < 0$ is the aggregate price sensitivity, and the
  unique solution exists because $B < 0$ ensures the clearing condition has a
  unique fixed point in $\mu_{m,t}$ by the implicit function theorem. Uniqueness
  of the equilibrium spread $\mu_t^*$ follows from the monotonicity of market
  clearing in $\mu_t$ (each sector's net demand is decreasing in price for
  demanders and increasing for suppliers).
...
\begin{proof}
  ...
  \textit{Step 2 (Effect of $\chi_t$ on $\mu_t^*$).}
  From the equilibrium demand ratio (derived from FOC~\eqref{eq:foc}):
  \[
    \frac{I_t^*}{O_t^*} = \left[\frac{1-\alpha_t}{\alpha_t}\cdot
    \chi_t^{\rho}\cdot\frac{R_{O,t}}{R_{I,t}}\right]^{\sigma}.
  \]
  A fall in $\chi_t$ reduces $(I_t^*/O_t^*)^\sigma$, which means a given
  quantity of $I_t$ produces less liquidity service. To maintain $L_t =
  \bar{L}_t$, agents must either obtain more $I_t$ (at higher cost) or
  substitute toward $O_t$. Because $O_t$ is in fixed supply in the conversion
  market (its supply is $B_t$-constrained), the scarcity of $O_t$ drives up
  $R_{O,t}$ or, equivalently, the cost of accessing $O_t$ via the conversion
  market rises. More precisely, the equilibrium condition from
  \eqref{eq:price-eq} gives:
  \[
    \frac{\partial\mu_{m,t}^*}{\partial\chi_t} = -\frac{1}{B}\cdot
    \frac{\partial}{\partial\chi_t}\sum_s \gamma_s X_{s,m,t}.
  \]
  When $U_t$ rises (holding $B_t$ fixed), the effective demand for $O_t$-providing
  counterparties rises, tightening the market. In the demand system, this
  tightening transmits through $\chi_t$: a lower $\chi_t$ reduces the effective
  supply of dollar liquidity, shifting the market-clearing $\mu_{m,t}^*$ upward.
  Formally, from the market clearing condition:
  \[
    \frac{\partial\mu_t^*}{\partial U_t}
    = \underbrace{\frac{\partial\mu_t^*}{\partial\chi_t}}_{\text{quality channel}}
      \cdot\underbrace{\frac{\partial\chi_t}{\partial p_t}}_{\chi'(p_t)>0}
      \cdot\underbrace{\frac{\partial p_t}{\partial U_t}}_{<0}.
  \]
  To establish the sign, note that when $\chi_t$ falls, the effective supply of
  dollar liquidity service falls, requiring a higher $\mu_t^*$ to clear the
  market (higher price to ration demand). Hence $\partial\mu_t^*/\partial\chi_t
  < 0$, and combining with $\chi'(p_t) > 0$ and $\partial p_t/\partial U_t < 0$
  gives $\partial\mu_t^*/\partial U_t > 0$.
...
\begin{proposition}[Non-linearity: Threshold in Market Tightness]
  ...
  Under Assumptions~\ref{ass:liq-req}--\ref{ass:regularity}, if $\chi$ is
  strictly convex (i.e.\ $\chi''(p) > 0$), the elasticity of $\mu_t^*$ with
  respect to $\theta_t$ is increasing in $\theta_t$. More precisely:
  \[
...
\begin{proof}
  ...
  (concave $p$ in $B$ but convex in $\theta$). Under $\chi'' > 0$ and
  $\Phi' < 0$, the second term is negative; but as $\theta \to \infty$,
  $p(\theta) \to 0$ and $\chi'(p) \to \chi'(0^+)$, while $|p'(\theta)|$
  grows without bound. The dominant term for large $\theta$ is
  $\Phi'(\chi)\chi'(p)p''(\theta)$, which is $\Phi'<0$ times $\chi'>0$ times
  $p''>0$, yielding a negative contribution. However, the overall sign is
  positive when the convexity of $\Phi$ in $\chi$ dominates.

  More directly: define $f(\theta) = -\mu_t^*(\theta) = -\Phi(\chi(p(\theta)))$.
  Then $f'(\theta) = -d\mu^*/d\theta < 0$ and the claim $d^2\mu^*/d\theta^2 > 0$
  is equivalent to $f''(\theta) < 0$, i.e.\ $-\mu^*(\theta)$ is concave in
  $\theta$, or $\mu^*$ is convex. This holds when the quality function $\chi$
  is convex in $p$ (spreads accelerate as quality deteriorates rapidly at the
  margin). The threshold $\theta^*$ is defined as the inflection point of
  $\mu^*(\theta)$ if $\chi$ is S-shaped (concave then convex), which is a
  sufficient but not necessary condition. Under a simpler assumption that $\chi$
  is log-convex (i.e.\ $\chi(p) = p^{\delta}$ with $\delta > 1$), convexity
  holds globally and $\theta^*$ can be set to any value without requiring an
  inflection point.
  ...
  of equilibrium). The non-linearity result additionally requires $\chi'' > 0$
  (imposed here as a sufficient condition; this is not part of
  Assumption~\ref{ass:quality} as stated, which only requires $\chi' > 0$).
  \hfill$\square$
\end{proof}
...
\begin{proposition}[Provider Heterogeneity]
  ...
  \end{enumerate}
  Under the demand-system specification~\eqref{eq:net-demand}, the price
  sensitivity of foreign bank demand $\beta_{FB} < 0$ is (in absolute value)
  smaller than the price sensitivity of hedge fund supply $\beta_{HF} > 0$:
  \[
    |\beta_{FB}| < \beta_{HF}.
  \]
\end{proposition}
...
\begin{proof}
  ...
  (c). For part (b): the hedging fund supply function $B_t^{HF} = \phi
  ...
  For the price-sensitivity ordering: foreign banks face inelastic demand for
  dollar funding because their payment obligations ($\bar{L}_t$) are largely
  predetermined (Assumption~\ref{ass:liq-req}). Hedge funds face no such
  constraint and enter only when the basis exceeds their entry cost. Hence the
  HF supply curve is more price-sensitive (steeper in $\mu_t$) than FB demand,
  giving $|\beta_{FB}| < \beta_{HF}$.

  \textit{Note:} The ordering $|\beta_{FB}| < \beta_{HF}$ is derived from the
  economic structure of each sector's problem rather than estimated; the
  empirical demand system in v8 Part 5 tests whether this inequality holds in
  data.
  \hfill$\square$
\end{proof}
...
\begin{proof}
  ...
  \textit{Part (b).} When $|\Lambda(\hat\mu)| > 1$ at some $\hat\mu$, the
  Poincar\'{e}--Hopf index theorem for scalar functions implies that there must
  be at least two additional fixed points surrounding $\hat\mu$ --- one on each
  side --- to maintain continuity and boundary conditions (as $F(\bar\mu) > 0$
  at the no-HF spread and $F(\mu) < 0$ at very low $\mu$ below $c^{HF}$).
  Specifically, by the intermediate value theorem applied to $F(\mu) = \Psi(\mu)
  - \mu$: if $F$ crosses zero from above (a stable equilibrium) then from below
  (unstable), then from above again (another stable equilibrium), the model
  admits at least three fixed points, of which two are stable. The low stable
  fixed point is the normal equilibrium ($\mu^{low}$) and the high stable fixed
  point is the run equilibrium ($\mu^{high}$).
...
\begin{itemize}[noitemsep]
  ...
  \item $\chi'' > 0$: quality is convex in matching probability. This is an
        additional condition used only in \cref{prop:nonlinear}; it is stated
        explicitly there and not inherited from the earlier assumptions.
